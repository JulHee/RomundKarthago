...
public class Joernson extends AIPlayer
{
	 ...
	 */
	public void kette (ArrayList<Knoten> temp,
					   Seite seite,
					   Knoten knot)
	{
	 ...
	 * @return
	 */
	public boolean checkme (ArrayList<Knoten> temp,
							Knoten k)
	{
		for ( Knoten z : temp )
		{
	 ...
	 */
	public boolean used (Knoten u,
						 ArrayList<ArrayList<Knoten>> p)
	{
	...
	public Boolean checkHistory (Knoten b)
	{
		...
		{
			return ! (
					mechanik.getMyGraph().getHistory().contains(
							mechanik.getMyGraph().ssuf( new Zug( meineSeite.toString() + " " + b.id ) )
									.convertToString() )
			);
		} catch ( Exception e )
		{
